...
## Time series evaluation Demer valley (Schulensmeer)
...
list.of.packages <- c("tidyverse", "sf", "stars", "mapview", "lubridate", "dplyr", "rpart", "rpart.plot", "leaflet", "mapedit", "scales", "ggplot2", "rstudioapi","tidyr","zoo","np","kernlab","leafem","viridis","cubelyr","terra","signal","abind","dygraphs","xts")
...
if (is.null(folder)){
  folder <- "G:/Gedeelde drives/Team_BioDiv/5_Projecten/2024_Biodiversa_habitatpilot/WP2_3/Inundation/HabPil_inundation_R data/Data/Habitat patches inundated/"
} 
...
```{r}
wetlands <- st_read(paste0(folder,"wetlands_schulensmeer.gpkg")) |> st_transform(32631) # Transform to local crs system (WGS 84 / UTM zone 31N - EPSG:32631)

grasslands <- st_read(paste0(folder,"/grasslands_schulensmeer.gpkg")) |> st_transform(32631) # Transform to local crs system (WGS 84 / UTM zone 31N - EPSG:32631)
...
obj <- read_stars(paste0(ts_folder,"/polygon_Demervallei_2.nc"), proxy = TRUE) # Region code: 1: Kloosterbeemden, 2: Schulensmeer, 3: Webbekomsbroek
obj2 <- read_stars(paste0(ts_folder,"/polygon_Demervallei_new_2.nc"), proxy = TRUE) 

obj3 <- read_stars(paste0(ts_folder,"/polygon_Demervallei_last_2.nc"), proxy = TRUE) 
...
```{r}
outputfolder <- "G:/Gedeelde drives/Team_BioDiv/5_Projecten/2024_Biodiversa_habitatpilot/WP2_3/Inundation/Validatie inundatie modellen/Tijdsreeksanalyse studiegebieden/output"
```
...
![](images/clipboard-2242592071.png)

![](images/clipboard-2922383982.png)
...
```{r}
# layer <- "OMWRGB22VL_vdc"# select the winter image of 2020
# layer2 <- "OMWRGB22VL"
# map <- leaflet() %>%
#   addWMSTiles(
#     wms_ortho,
#     layers = layer,
#     options = WMSTileOptions(format = "image/png", transparent = FALSE), group = "vdc") %>%
#   addWMSTiles(
#     wms_ortho,
#     layers = layer2,
#     options = WMSTileOptions(format = "image/png", transparent = FALSE), group = "ortho") %>%
...
Classification based on Jussila's model:
...
Comparison with the WiW classification:
