---
title: "Datacube Analysis Demervalley - Schulensmeer"
author: 
...
output: 
  html_document:
    ...
    code_folding: show
knit: (function(input, ...) {rmarkdown::render(input, output_file="./source/hydrology/analysis/Datacube_Analysis_Demervalley-SM.html")})
---
...
![](G:/Gedeelde%20drives/Team_BioDiv/5_Projecten/2024_Biodiversa_habitatpilot/WP2_3/INBO_Github_Biodiversa_Habitat_Pilot/Habitat_condition_indicators/source/hydrology/media/images/clipboard-2242592071.png)

![](G:/Gedeelde%20drives/Team_BioDiv/5_Projecten/2024_Biodiversa_habitatpilot/WP2_3/INBO_Github_Biodiversa_Habitat_Pilot/Habitat_condition_indicators/source/hydrology/media/images/clipboard-2922383982.png)
...
#### No temporal interpolation {.tabset}
...
##### Inundation (Jussila model), no temporal interpolation
```{r}
# Example: show first AOI results
results[[1]]$plots$inundation
```

##### STR soil moisture index, no temporal interpolation

```{r}
results[[1]]$plots$STR
```

##### Dygraph of all wetness/moisture indices, no temporal interpolation

```{r}
results[[1]]$dygraphs$indices
```

##### Dygraph of both inundation classification models, no temporal interpolation

```{r}
results[[1]]$dygraphs$inundation
```

#### With temporal interpolation {.tabset}
...
##### Inundation (Jussila model), rasters, temporal linear interpolation
```{r}
results2[[1]]$plots$inundation_raster
```

##### Inundation (Jussila model), temporal linear interpolation
```{r}
results2[[1]]$plots$inundation_graph
```

##### STR soil moisture index, temporal linear interpolation
```{r}
results2[[1]]$plots$STR
```

##### NDMI soil moisture index, temporal linear interpolation
```{r}
results2[[1]]$plots$NDMI
```

##### B11 (SWIR band), temporal linear interpolation
```{r}
results2[[1]]$plots$B11
```

##### Dygraph of all wetness/moisture indices, temporal linear linterpolation
```{r}
results2[[1]]$dygraphs$indices
```

##### Dygraph of both inundation classification models, temporal linear linterpolation
...
##### Dygraph of Jussila inundation classification model, temporal linear linterpolation
